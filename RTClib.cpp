...
*/
/**************************************************************************/
static uint8_t dec2Bcd(uint8_t value){return (value / 10 * 16 + value % 10); }
...
/**************************************************************************/
/*!
    @brief  Set minute and hour alarm
*/
/**************************************************************************/

void RTC_PCF8523::setAlarm(uint8_t Hour, uint8_t Minute){
	
	if(Hour == 0){
		Wire.beginTransmission(PCF8523_ADDRESS);
        Wire.write(PCF8523_HOUR_ALARM);
		Wire.write(PCF8523_ALARM_REG_OFF);
		Wire.endTransmission();
	}
	
	else if(Minute == 0){
		Wire.beginTransmission(PCF8523_ADDRESS);
        Wire.write(PCF8523_MINUTE_ALARM);
		Wire.write(PCF8523_ALARM_REG_OFF);
		Wire.endTransmission();
	}
  ...
  byte aMinute, aHour;
  aMinute = dec2Bcd(aMinute);
  aMinute = aMinute | 100000000;
  
  aHour = dec2Bcd(aHour);
  aHour = aHour | 100000000;
 ...
 // Wire.write(aHour);
  ...
  Wire.endTransmission();
  Wire.requestFrom(PCF8523_ADDRESS, 1);
  interruptFlag = Wire.read();
  interruptFlag = interruptFlag & B00001000; // isolate the alarm flag bit
  if (interruptFlag == B00001000) // alarm on?
  {
    // clear AF 
    Wire.beginTransmission(PCF8523_ADDRESS);
...
/**************************************************************************/
/*!
    @brief  Enable alarm interrupt, Desable CLOCKOUT 
*/
/**************************************************************************/

void RTC_PCF8523::enableAlarm(void){
	
	/*Disable CLKOUT*/
  Wire.beginTransmission(PCF8523_ADDRESS);
  Wire.write(PCF8523_CLKOUTCONTROL);
  Wire.write(PCF8523_CLKOUT_OFF);
  Wire.endTransmission();
  
  /*Enable alarm interrupt*/
  Wire.beginTransmission(PCF8523_ADDRESS);
  Wire.write(PCF8523_CONTROL_1);
  Wire.write(PCF8523_ALARM_AIE);
  Wire.endTransmission();
...
/**************************************************************************/
/*!
    @brief  Alarm interrupt flag clear 
*/
/**************************************************************************/
/*
void RTC_PCF8523::clearAlarm_INT(void){	  
  Wire.beginTransmission(PCF8523_ADDRESS);
  Wire._I2C_WRITE((byte)PCF8523_CONTROL_2);
  Wire._I2C_WRITE((byte)0x00);
  Wire.endTransmission();
}*/

void RTC_PCF8523::countDown_Enable(uint8_t timeUnit){
	  /*enable timer count down interrupt CTAIE*/
  Wire.beginTransmission(PCF8523_ADDRESS);
  Wire.write(PCF8523_CONTROL_2);
  Wire.write(0x02);
  Wire.endTransmission();

  /*/enable interrupt pulse,timer A is configured as countdown timer and desable CLOCK OUT on pin 7*/
  Wire.beginTransmission(PCF8523_ADDRESS);
  Wire.write(PCF8523_CLKOUTCONTROL);
  Wire.write(0xBA);
  Wire.endTransmission();

  /*set count down time unit in second ,minute or in hour*/
  Wire.beginTransmission(PCF8523_ADDRESS);
  Wire.write(TMR_A_FREQ_CONTROL);
  Wire.write(timeUnit);
  Wire.endTransmission();
}

void RTC_PCF8523::setCountDown(uint8_t Time) {
	/*set count down time*/
  Wire.beginTransmission(PCF8523_ADDRESS);
  Wire.write(TMR_A_REG);
  Wire.write(Time);
  Wire.endTransmission();
}

boolean RTC_PCF8523::countDownCheck(void){
	
	/*keep track of time counting down and clear interrupt flag */
  byte test;
  Wire.beginTransmission(PCF8523_ADDRESS);
  Wire.write(PCF8523_CONTROL_2);
  Wire.endTransmission();
  Wire.requestFrom(PCF8523_ADDRESS, 1);
  test = Wire.read();
  test = test & B01000000; // isolate the alarm flag bit
  if (test == B01000000) // alarm on?
  {
	return true;
  }else{
	return false;
